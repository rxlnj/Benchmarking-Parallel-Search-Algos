...
time_patterns = {
    ...
    "seq": r"Search time: (\d+\.\d+) milliseconds"
}
...
def run_implementation(impl_name, use_mpi, dataset_size):
    ...
    if use_mpi:
        cmd = ["mpirun", "-n", "4", "bin/" + impl_name, dataset_path]
    else:
    ...
    elif "pthread" in impl_name:
        pattern = time_patterns["pthreads"]
    else:
        pattern = time_patterns["seq"]
    ...
    if match:
        ...
        execution_time = "N/A"
        print("[ERROR] TIME WAS NOT SAVED! [",impl_name,"]")
...
with open(output_filename, "w") as output_file:
    output_file.write("Dataset length,Binary_MPI_time,Linear_MPI_time,Binary_omp_time,Linear_omp_time,Binary_pthreads,Linear_pthreads, Binary_Sequential, Linear_Sequential\n")
    for dataset_size in dataset_sizes:
        ...
        binary_time = run_implementation("binary_seq", False, dataset_size)
        linear_time = run_implementation("linear_seq", False, dataset_size)

        output_file.write(f"{dataset_size},{binary_mpi_time},{linear_mpi_time},{binary_omp_time},{linear_omp_time},{binary_pthread_time},{linear_pthread_time},{binary_time},{linear_time}\n")
